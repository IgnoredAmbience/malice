...
toSymbInstr (SGet n) = [UnMOp MPop (Reg EAX), BinMOp MMov (Reg EBX) (ConstName n), JmpMOp MCall (Lbl "bounds_check") -- Get the index and name
                     , BinMOp MMov (Reg EAX) (IndirectScale (Reg EBX) Four (Reg EAX)), UnMOp MPush (Reg EAX)] -- Get the value itself and push it

toSymbInstr (SPut n) = [UnMOp MPop (Reg EAX), BinMOp MMov (Reg EBX) (ConstName n), JmpMOp MCall (Lbl "bounds_check"), UnMOp MPop (Reg ECX) -- Get the value, name and index
